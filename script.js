...
function calculateScore(selected) {
  return selected.reduce((total, item) => {
    return total + (item.selected === item.correct ? 1 : 0);
  }, 0);
}
...
quizForm.addEventListener('submit', (event) => {
  ...
  if (!selected) {
    resultBox.textContent = 'Beantwoord alle vragen voordat je verstuurt.';
  ...
  resultBox.textContent = `Je score: ${score}/${total}. Je e-mailprogramma wordt geopend om je uitslag te verzenden.`;
